...
public class SendNotificationsAboutAppointmentInTwoHoursJob : IJob
{
    private readonly IAppointmentsRepository _appointmentsRepository;
    private readonly INotificationsSenderService _notificationsSenderService;

    public SendNotificationsAboutAppointmentInTwoHoursJob(IAppointmentsRepository appointmentsRepository,
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var appointments = await _appointmentsRepository.GetAll()
            .Include(p => p.Patient)
            .Include(d => d.Dentist)
            .Where(x => x.Date.Date == now.Date && x.PatientId != null
            && TimeOnly.FromDateTime(x.Date) == TimeOnly.FromDateTime(now).AddHours(twoHours))
            .ToListAsync();
